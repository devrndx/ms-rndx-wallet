...
const rndxAddr = "0xFf1Bf7a212012ee903fBb5a3adB4A9a31f371EF7";
...
actions.transferAdmin = async function(req, res, next) {
    ...
    const account = await contractService.getAccount(pk);
    ...
    contractService.transfer(account, toAddr, amount)
        .then((receipt) => {
        ...
        .then(async() => {
            ...
            log.info("toAddr :" + toAddr);
            log.info("address :" + account.address);
            const receipt = await contractService.lockWallet(account, toAddr);
            if (receipt === undefined) {
                const msg = "Already Locked:" + receipt.transactionHash + ":" + toAddr + ":0";
                log.info(msg);
                req.app.get("io").emit("TxStateChanged", msg);
            } else {
                const msg = "Lock Done:" + receipt.transactionHash + ":" + toAddr + ":0";
                log.info(msg);
                req.app.get("io").emit("TxStateChanged", msg);
            }
        })
