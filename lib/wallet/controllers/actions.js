...
actions.transferAdmin = async function(req, res, next) {
    ...
    const receipt = await contractService.transfer(account, toAddr, amount);
    let msg = "Done:" + receipt.transactionHash + ":" + toAddr + ":0";
    log.info(msg);
    req.app.get("io").emit("TxStateChanged", msg);

    await contractService.lockWallet(account, toAddr);
    msg = "Lock Done:" + receipt.transactionHash + ":" + toAddr + ":0";
    log.info(msg);
    req.app.get("io").emit("TxStateChanged", msg);
