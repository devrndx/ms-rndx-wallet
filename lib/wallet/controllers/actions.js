...
actions.airDrop = async function(req, res, next) {
    ...
    contractService
        ...
        .then(async(ownerAcnt) => {
            log.info("Address : " + toAddr);
            log.info("amount : " + amount);
            log.info("Owner : " + ownerAcnt.address);


            return await contractService.transfer(ownerAcnt, toAddr, amount);
        })
        .then(async(receipt) => {
            const balance = await contractService.balanceOf(toAddr);

            const response = { status: "ok", address: toAddr, balance: balance };
            res.status(200).json(response);
        })
        .catch((error) => {
            log.error(error.message);
